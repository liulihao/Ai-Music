...
<!--這裡用的css-->
<link href="../CSSfile/member_page_music.css" rel="stylesheet" type="text/css" />
<!--jump window-->
...
<div id="showid" class="font10 style4" align="left">
    ...
    <a href="member_page.jsp?user=<%=user_id%>">首頁 </a> <span class="style12">&gt;&nbsp;</span><a href="admin_page_manage_activity.jsp"></a><a href="member_page.jsp?user=<%=user_id%>"><%=user_id%></a><!--這裡需要抓id自動產生帳號名稱-->
   ...
   <div id="bigleft" class="font10 style4">
     ...
     <title><%=user_data.getString("user_nkname")%>的音樂作品</title>
     <!--aboutme-block-->
...
%>
  ...
  <div  id="bigright" class="font10" >
    ...
    <%
	   Statement classify=con.createStatement();//抓歌曲風格 NATURAL JOIN查詢
	   String classify_sql="SELECT * from user_track_classify where user_id='"+user_id+"'";
       ResultSet classify_data;
       classify_data=classify.executeQuery(classify_sql);
	   while(classify_data.next()){
	     String classify_name=classify_data.getString("classify_name");
		 int classify_id=classify_data.getInt("classify_id");
    %>
    <!--分類間隔div-->
    <div class="space"> 
    <!----------分類title----------><!----------分類end---------->    
     <div  align="left" class="music-title roundedCorners" style="position:relative">
       <table width="652" border="0">
         <tr>
           <td width="522" class="title-name1 style17"><%=classify_name%></td>
           <td width="120" class="title-style1 font10 style4" align="right"><div style="padding-right:11px">&lt;音樂風格&gt;</div></td>
         </tr>
       </table>
       <div class="roundedCorners bar"></div>
     </div>
     <%
  	 sql="select * from track where user_id='"+user_id+"' and classify_id="+classify_id;
  	 ResultSet song;
 	 song=stmt.executeQuery(sql);
  	 	while(song.next()){
	 		 ...
	 		 Statement track_type=con.createStatement();//抓歌曲風格 
...
%>  
     <div  align="left" class="font10 tablebtmgray style4 song" > 
       <table width="652" border="0">
          <tr>
             <td width="370" class="title-style font10 style4"><a href="member_page_music_song.jsp?music_id=<%=track_id%>"><%=track_name%></a></td>          
             <td width="272" class="title-style font10 style4" align="right">&lt;<%
    ...
    <div  align="left" class="music-title roundedCorners" style="position:relative">
       <table width="652" border="0">
         <tr>
           <td width="521" class="title-name1 style17">未分類</td>
                      <td width="120" class="title-style1 font10 style4" align="right"><div style="padding-right:11px">&lt;音樂風格&gt;</div></td>
         </tr>
       </table>
       <div class="roundedCorners bar"></div>
     ...
     <%
	    Statement undefine=con.createStatement();
	 	sql="SELECT * FROM track WHERE classify_id=0 AND user_id='"+user_id+"'";
		rs=undefine.executeQuery(sql);
		while(rs.next()){
		  int undefine_track_id=rs.getInt("track_id");
		  String undefine_sql="select * from track_type,music_type where track_type.music_id=music_type.music_id AND track_type.track_id="+undefine_track_id;
		  Statement undefine_genre=con.createStatement();
		  ResultSet undefine_genre_data=undefine_genre.executeQuery(undefine_sql);
	 %>
     <div  align="left" class="font10 tablebtmgray style4 song" > 
       <table width="652" border="0">
          <tr>
             <td width="370" class="title-style font10 style4"><a href="member_page_music_song.jsp?music_id=<%=undefine_track_id%>"><%=rs.getString("track_name")%></a></td>          
             <td width="272" class="title-style font10 style4" align="right">&lt;<%
			 	if(!undefine_genre_data.next()){//如果找不到風格的話 顯示未定義
		 			 out.print("未定義");
				 }
				 else{//否則 則去把風格抓出來
				 	undefine_genre_data.previous();
					 while(undefine_genre_data.next()){//undefine_genre_data.next()
					 out.print(undefine_genre_data.getString("music_type.music_genre"));
					 out.print("&nbsp;");
					 }//end fo undefine_genre_data.next()
				 }

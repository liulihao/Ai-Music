...
<!--這裡用的css-->
<link href="../CSSfile/member_page_friends.css" rel="stylesheet" type="text/css" />
<!--jump window-->
...
<div id="container" align="left">
  ...
  <div id="bigleft" class="font10 style4">
    ...
    <title><%=user_data.getString("user_nkname")%>的朋友</title>
    <!--aboutme-block-->
...
<!--------------------------好友 ---------------------------------->   
<div id="friends-title" align="left" class="style4 roundedCorners style17 "style="position:relative"  >我的朋友
  <div class="roundedCorners bar"></div>
</div>
<%//好友列表頁碼
    int PageSize=25;//設定每張網頁顯示的筆數
	...
	if(ToPage!=null){//判斷是否可正確取得ToPage參數 可取得則表示JSP網頁應顯示特定分頁紀錄的敘述
	 ...
	 sql="select COUNT(*) from friend where user_id='"+user_id+"'";
	...
	PageCount=(RowCount+PageSize-1)/PageSize;//計算顯示的頁數
	 if(ShowPage>PageCount){
	  ...
	  sql="select * from friend where user_id='"+user_id+"' AND friend_check = 1 ORDER BY friend_time DESC";
...
<!-- 頁碼-->
    ...
    <form action="member_page_friends.jsp?user=<%=user_id%>" method="POST">
     <table  width="651" border=0>
   <tr>
       <td width="129"><div align="center">目前頁次:<font color=red><%=ShowPage%></font>/<font color=blue><%=PageCount%></font> </div></td>
		<td width="65"><div align="center"><a href=member_page_friend.jsp?user=<%=user_id%>&ToPage=1>[第一頁]</a> </div></td> 
		<td width="65"><div align="center"><a href=member_page_friend.jsp?user=<%=user_id%>&ToPage=<%=ShowPage-1%>>[上一頁]</a> </div></td>
		<td width="65"><div align="center"><a href=member_page_friend.jsp?user=<%=user_id%>&ToPage=<%=ShowPage+1%>>[下一頁]</a> </div></td>
		<td width="85"><div align="center"><a href=member_page_friend.jsp?user=<%=user_id%>&ToPage=<%=PageCount%>>[最後一頁]</a> </div></td>
...
%>    
  <div class="friend-block" align="center"> 
      <a href="member_page.jsp?user=<%=friend_id%>"><div style="margin-bottom:5px;border:1px solid #E0E0E0;width:90px;height:90px"><img src="user/<%=friend_id%>/<%=friend_pic%>" width="90" height="90" /></div></a>

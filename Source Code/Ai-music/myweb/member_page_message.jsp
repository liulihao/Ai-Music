...
<!--這裡用的css-->
<link href="../CSSfile/member_page_message.css" rel="stylesheet" type="text/css" />
<!--css end-->
...
<div id="bigleft" class="font10 style4">
  ...
  <title><%=user_data.getString("user_nkname")%>的留言版</title>
  <!--aboutme-block-->
...
<%//留言板 
	...
	if(ToPage!=null){//判斷是否可正確取得ToPage參數 可取得則表示JSP網頁應顯示特定分頁紀錄的敘述
	 ...
	 sql="select COUNT(*) from message where user_id='"+user_id+"'";
	...
	PageCount=(RowCount+PageSize-1)/PageSize;//計算顯示的頁數
	 if(ShowPage>PageCount){
	  ...
	  sql="select * from message,users where users.user_id = message.post_id AND message.user_id='"+user_id+"'  ORDER BY time DESC";
...
function message_post() {
    Dialog.confirm("1",{className: "alphacube", width:325 ,height:327,closable:true,url: "member_message_leave.jsp?user=<%=user_id%>&web_url=<%=web_url%>&login=<%=login_user%>"})
  }

function message_reply(id) {
	var id=id;
    Dialog.confirm("2",{className: "alphacube", width:320 ,height:249,closable:true,url: "member_message_reply.jsp?user=<%=user_id%>&web_url=<%=web_url%>&id="+id})
  }
</script>


    <!-- title-->
    <div  id="title" align="left" class=" roundedCorners style17 " style="position:relative" >留言板
        <div class="roundedCorners bar"></div>
      <!---end of title--->
    </div>
    <!-- 新增留言-->
    <% //如果登入帳號與會員空間帳號相同 則顯示回覆留言 
	 
	if(!user_id.equals(login_user) && login_user != null){
	    out.print("<div class='new_message '>");
		out.print("<div align='center' class=' new-message new'>");
		out.print("<a href='#' onClick='message_post();'>新增留言</a>");
		out.print("</div>");
		out.print("</div>");
	}
	else if (login_user == null){
		out.print("<FONT color='red' >您尚未登入，無法留言</FONT>");
	} 
%>
    <!--把會員空間uesr_id放入函式-->
        <!--把該會員空間的會員帳號user_id當索引到留言區-->
        <!--end of 新增留言-->
   
    <!--頁次-->
    <div align="left" style="margin:20px 0">
    <form action="member_page_message.jsp?user=<%=user_id%>" method="POST">
      <table width="651" border=0>
        <tr>
          <td width="138">目前頁次:<font color=red><%=ShowPage%></font>/<font color=blue><%=PageCount%></font> </td>
          <td width="70" ><a href=member_page_message.jsp?user=<%=user_id%>&ToPage=1>[第一頁]</a></td>
          <td width="70" ><a href=member_page_message.jsp?user=<%=user_id%>&ToPage=<%=ShowPage-1%>>[上一頁]</a> </td>
          <td width="70"><a href=member_page_message.jsp?user=<%=user_id%>&ToPage=<%=ShowPage+1%>>[下一頁]</a> </td>
          <td width="91"><a href=member_page_message.jsp?user=<%=user_id%>&ToPage=<%=PageCount%>>[最後一頁]</a> </td>
          <td >輸入頁次:
...
%>
    ...
    <div id='reply-outside'>
      <!--內框1-->
      <div id='reply-inside1' align='left'>
        <table width='640' border='0'>
          <tr>
            <td colspan='4' rowspan='4'><a href="member_page.jsp?user=<%=rs.getString("post_id")%>"><img src='user/<%=rs.getString("post_id")%>/<%=rs.getString("users.user_pic")%>' width='90' height='90'></a></td>
            <td width='408' class="font10">發表會員:<a href="member_page.jsp?user=<%=rs.getString("post_id")%>"><%=rs.getString("users.user_nkname")%></a></td>
            
            <td width="69" align="right"><span class="font10">
              <% //如果登入帳號與會員空間帳號相同 則顯示回覆留言與刪除留言
			int id=rs.getInt("id");
			if(user_id.equals(login_user)){
			out.print("<a href='#' onClick='message_reply("+id+");'>[回覆]</a>");
		 }
	   %>
            </span></td>
            <td width="57"><span class="font10">
              <% //如果登入帳號與會員空間帳號相同 則顯示回覆留言與刪除留言
			
			if(user_id.equals(login_user)){
			out.print("<a href='message/message_delete.jsp?user="+user_id+"&id="+id+"' onClick='return delete_message()'>[刪除]</a>");
		 }
	   %>
            </span></td>
          </tr>
          <tr>
            <td class="font10">主題:<%=rs.getString("subject")%></td>
          </tr>
          <tr>
            <td class="font10">發表時間:<%=rs.getDate("time")%> <%=rs.getTime("time")%></td>
          </tr>
          <tr>
            <td class="font10">&nbsp;</td>
          </tr>
      ...
      <!--內框2-->
      <div id='' style="margin-left:10px;"align='left'><%=rs.getString("memo")%>
        <!--end of 內框2-->
      </div>
      <!--內框三-->
      <%//回覆留言 若有留言才顯示出來
...
//if2
%>
				<div id='reply-inside2' align='left'>
				  <table width="500" cellpadding="0" cellspacing="5">
                    <tr>
                      <td><strong>回覆留言:</strong></td>
                    </tr>
                    <tr>
                      <td><table width="0">
                        <tr>
